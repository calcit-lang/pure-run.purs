...
procHashMap :: (Array CalcitData) -> Effect CalcitData
procHashMap xs = do
  ys <- foldTuples [] xs
  pure (CalcitMap (Map.fromFoldable ys))
  where
  foldTuples ::
    Array (Tuple CalcitData CalcitData) ->
    Array CalcitData ->
...
procAssoc :: (Array CalcitData) -> Effect CalcitData
procAssoc xs = case (xs !! 0), (xs !! 1), (xs !! 2) of
...
procDissoc :: (Array CalcitData) -> Effect CalcitData
procDissoc xs = case (xs !! 0), (xs !! 1) of
...
procMerge :: (Array CalcitData) -> Effect CalcitData
procMerge xs = case (xs !! 0), (xs !! 1) of
...
-- TODO use set
procToPairs :: (Array CalcitData) -> Effect CalcitData
procToPairs xs = case xs !! 0 of
  Just (CalcitMap a) ->
    pure
      ( CalcitList
          ( Array.mapMaybe
...
-- TODO pairs-map
procMapKv :: (Array CalcitData) -> Effect CalcitData
procMapKv xs = case (xs !! 0), (xs !! 1) of
