...
import Calcit.Builtin.List (procAppend, procConcat, procCount, procDrop, procFoldl, procList, procMap, procMapMaybe, procNth, procPrepend, procRange, procRepeat, procReverse, procSlice, procTake)
...
coreNsDefs = Map.union coreNsSyntaxes coreDefs
  ...
  coreDefs =
    Map.fromFoldable
      ...
      , (Tuple "range" (CalcitFn "range" (genv3UUID "faked_range" uidSeed) procRange))
      , (Tuple "repeat" (CalcitFn "repeat" (genv3UUID "faked_repeat" uidSeed) procRepeat))
      , (Tuple "reverse" (CalcitFn "reverse" (genv3UUID "faked_reverse" uidSeed) procReverse))
      , (Tuple "append" (CalcitFn "append" (genv3UUID "faked_append" uidSeed) procAppend))
      , (Tuple "prepend" (CalcitFn "prepend" (genv3UUID "faked_prepend" uidSeed) procPrepend))
      , (Tuple "take" (CalcitFn "take" (genv3UUID "take" uidSeed) procTake))
      , (Tuple "drop" (CalcitFn "drop" (genv3UUID "drop" uidSeed) procDrop))
      -- strings
